...
def main() -> int:
    ...
    vcan_setup = run(["bash", "scripts/setup_vcan0.sh"]) if args.setup_vcan else {"skipped": True, "reason": "pass --setup-vcan to create vcan0"}
    ...
    checks = {
        ...
        "python_modules": {"can": module_ok("can"), "serial": module_ok("serial"), "bleak": module_ok("bleak")},
        "repo_files": {
    ...
    report = {
        ...
        "interface_status": {"can": run(["ip", "-details", "link", "show", can_iface]), "vcan": run(["ip", "-details", "link", "show", vcan_iface])},
    ...
    primary_found = bool(port_map.get("heltec")) if profile == "heltec" else bool(port_map.get("nrf_ble"))
    report["summary"] = {
