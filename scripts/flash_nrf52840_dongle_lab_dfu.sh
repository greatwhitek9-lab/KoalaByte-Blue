...
if ! command -v nrfutil >/dev/null 2>&1; then
  ...
  echo "Build artifact is ready at: $HEX" >&2
...
echo "Packaging nRF52840 Dongle DFU zip"
...
echo "Flashing nRF52840 Dongle over serial DFU port: $DFU_PORT"
nrfutil dfu usb-serial -pkg "$PACKAGE" -p "$DFU_PORT"

echo "Dongle DFU complete. Scan for BLE device name: EarTag-TX-Lab"
