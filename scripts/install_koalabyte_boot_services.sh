...
CHECK_ONLY=0
SERVICE_USER="${KOALABYTE_SERVICE_USER:-${USER:-pi}}"
...
if [[ "${CHECK_ONLY}" == "1" ]]; then
  echo "KoalaByte boot service templates are present."
  exit 0
fi
...
echo "Installed KoalaByte boot services. Start with: sudo systemctl start koalabyte-menu.service"
