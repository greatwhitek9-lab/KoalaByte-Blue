...
setup_anteater_for_selected_mode() {
  if [[ "${RUN_ANTEATER}" != "1" && "${RUN_SMOKE}" != "1" ]]; then
    return 0
  fi
  echo
  echo "== AntEater readiness check: passive triage integration, no live BLE scan =="
  mkdir -p logs/anteater
  if [[ "${BUILD_ONLY}" == "1" ]]; then
    echo "Build-only mode: validating AntEater import path only."
  fi
...
status = {
    ...
    "one_shot_flasher_check": True,
    "live_scan_started": False,
    "node_log": str(DEFAULT_NODE_LOG_PATH),
    "source": "one-shot-flasher-readiness",
    "safety": {
...
PY
  ...
  echo "AntEater ready. Status written to logs/anteater/anteater_status.json"
}
