...
The Pi 3B+ defaults are intentionally memory-conservative: a 768-token context,
64 predicted tokens, and two retained dialogue turns. Values may be overridden
in /etc/koalabyte-blue/killerkoala.env after deployment.
EOF
}
...
write_status() {
  ...
  python3 - "${STATUS_PATH}" "${status}" "${reason}" "${ESP32_MIC_PORT}" \
    ...
    "${KILLERKOALA_TTS_VOICE}" "${KILLERKOALA_ENV_FILE}" "${KILLERKOALA_LLM_NUM_CTX}" <<'PY'
import json, sys, time
(path, status, reason, port, service, user, group, model, voice, env_file, num_ctx) = sys.argv[1:]
payload = {
    ...
    "private_environment_file": env_file, "required_for_install": False,
...
write_status ok "Voice bridge installed with Pi 3B+ memory limits and William TTS."
