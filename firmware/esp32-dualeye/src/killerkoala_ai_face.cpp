...
static void drawEye(int x,int y,uint16_t fg,bool left){
  ...
  int pulse=(speaking||wake||success)?(frameNo<12?frameNo:24-frameNo):0;
  int jitter=action?((frameNo%3)-1)*3:0;
  int rx=46+pulse/2, ry=18+(speaking?(frameNo%5):0);
...
static void drawEyes(){
  ready(); int w=tft.width(),h=tft.height(),cy=h/2;
...
void showKillerKoalaAiFace(const char *state,const char *message,const char*,const char*,int,int duration_ms){
  ...
  active=true; frameNo=0; lastMs=0; untilMs=millis()+(uint32_t)(duration_ms>0?duration_ms:KILLERKOALA_FACE_DEFAULT_DURATION_MS); drawEyes();
}
...
void tickKillerKoalaAiFace(){if(!isKillerKoalaAiFaceActive())return; uint32_t now=millis(); if(now-lastMs<90)return; lastMs=now; frameNo=(frameNo+1)%24; drawEyes();}
#else
