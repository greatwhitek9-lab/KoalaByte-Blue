...
static void drawFuzzyCheeks(int cx,int cy,int bob){
  // SSD1306 is one-bit, so the grey fuzzy cheek look is dithered pixels.
  for(int side=-1;side<=1;side+=2){
    int base=cx+(side*32);
    for(int r=0;r<3;r++){
      int yy=cy+2+bob+(r*4);
      oled.drawPixel(base-(side*18),yy);
      oled.drawPixel(base-(side*10),yy+2);
      oled.drawPixel(base+(side*8),yy+1);
      oled.drawPixel(base+(side*18),yy+3);
    }
    oled.drawCircle(base,cy+9+bob,13);
    oled.drawCircle(base,cy+9+bob,18);
  }
}
...
static void drawSolidMouth(int cx,int cy,int bob){
  ...
  if(eq(stateName,"speaking")){
    int open=5+beat;
    int width=46+(beat/2);
    oled.drawRBox(cx-width/2,cy-open+bob,width,open*2,7); // solid mouth fill
    oled.setDrawColor(0);
    oled.drawRBox(cx-width/2+8,cy-open/2+bob,width-16,max(3,open),4); // dark inner cutout
    oled.drawHLine(cx-width/2+7,cy+bob,width-14);
    oled.setDrawColor(1);
  }else if(eq(stateName,"wake")||eq(stateName,"listening")){
    int nudge=(frameNo%6)-3;
    oled.drawRBox(cx-29,cy-3+nudge+bob,58,6,3);
  }else if(eq(stateName,"thinking")){
    for(int x=-36;x<=36;x+=12)oled.drawDisc(cx+x,cy+bob+(((x/12+frameNo)%2)?3:-3),3);
  }else if(eq(stateName,"action")){
    int sweep=(frameNo%16)-8;
    oled.drawRBox(cx-39,cy-4+bob,78,8,3);
    oled.setDrawColor(0);
    for(int x=-32;x<=32;x+=16)oled.drawVLine(cx+x+sweep/3,cy-8+bob,16);
    oled.setDrawColor(1);
  }else if(eq(stateName,"success")){
    int lift=(beat>6)?2:0;
    oled.drawLine(cx-34,cy-lift+bob,cx-15,cy+8-lift+bob); oled.drawLine(cx-15,cy+8-lift+bob,cx+15,cy+8-lift+bob); oled.drawLine(cx+15,cy+8-lift+bob,cx+34,cy-lift+bob);
    oled.drawLine(cx-34,cy+1-lift+bob,cx-15,cy+9-lift+bob); oled.drawLine(cx+15,cy+9-lift+bob,cx+34,cy+1-lift+bob);
  }else if(eq(stateName,"error")||eq(stateName,"blocked")){
    int dip=(beat>6)?2:0;
    oled.drawLine(cx-34,cy+7+dip+bob,cx-15,cy-2+dip+bob); oled.drawLine(cx-15,cy-2+dip+bob,cx+15,cy-2+dip+bob); oled.drawLine(cx+15,cy-2+dip+bob,cx+34,cy+7+dip+bob);
    oled.drawLine(cx-34,cy+9+dip+bob,cx-15,cy+dip+bob); oled.drawLine(cx+15,cy+dip+bob,cx+34,cy+9+dip+bob);
  }else{
    oled.drawRBox(cx-27,cy-2+bob,54,5,2);
  }
}
...
static void drawDefinedSnout(){
  ...
  // Dithered fuzzy grey cheeks around the muzzle.
  drawFuzzyCheeks(cx,cy,bob);

  // Bright muzzle pads and bridge.
  ...
  // Solid black nose: erase a nose-shaped hole inside a bright bridge.
  oled.drawBox(cx-11,cy-18+bob,22,10);
  oled.setDrawColor(0);
  oled.drawRBox(cx-10,cy-19+bob,20,11,4);
  oled.setDrawColor(1);
  oled.drawPixel(cx-5,cy-15+bob); oled.drawPixel(cx+5,cy-15+bob); // nostril highlights
  oled.drawVLine(cx,cy-8+bob,8);

  oled.drawRFrame(cx-43,cy-9+bob,86,25,7);
  drawSolidMouth(cx,cy+3,bob);
}
...
void setup(){
  ...
  Wire.begin(KOALA_OLED_SDA,KOALA_OLED_SCL); oled.begin(); oled.setContrast(220);
