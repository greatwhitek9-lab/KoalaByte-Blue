...
- `ASSEMBLY_AND_FLASHING_INSTRUCTIONS_RevA5_BUTTONS.md` — updated in-repo RevA16 assembly, flashing, button, boot-screen, jungle-menu, Ear Tag TX Lab, and local BlueZ procedure
...
- `scripts/check_boot_animation_config.py` — RevA16 repository validation check
...
The static production ZIP/PDF names still say RevA1 because they describe the no-custom-PCB hardware package. The live repository now contains newer RevA16 software/firmware additions, including nRF Connect SDK / Zephyr firmware build helpers and Koala BlueZ local helper commands. Use the latest repository scripts and docs when flashing or installing software.
