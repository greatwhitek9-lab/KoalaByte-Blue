...
PRIMARY_SOURCE = "heltec-t114"


def _clean_addr(value: Any) -> str:
    return str(value or "").strip().upper()


def normalize_ble_event(payload: dict[str, Any], *, default_source: str = "unknown") -> dict[str, Any]:
    """Return a stable KoalaByte BLE observation shape.

    The Heltec T114 nRF52840 is considered authoritative when duplicate
    observations arrive from secondary ESP32-S3 or Raspberry Pi BlueZ nodes.
    This helper keeps all sources in one log format without performing any
    pairing, connection, write, disruption, or offensive workflow.
    """
    ...
    event = {
        ...
        "source": source,
        "role": role,
        "addr": _clean_addr(payload.get("addr") or payload.get("address")),
...
def event_identity(event: dict[str, Any]) -> str:
    addr = _clean_addr(event.get("addr"))
    if addr:
        return addr
