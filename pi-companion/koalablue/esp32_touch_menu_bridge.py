...
class Esp32TouchMenuBridge:
    ...
    def calibration_command(self, *, screen_w: int = 240, screen_h: int = 240, row_height: int = 40, visible_rows: int = 6) -> str:
        return json.dumps({"type": "menu_frame", "screen_w": screen_w, "screen_h": screen_h, "row_height": row_height, "visible_rows": visible_rows})

    def _log(self, payload: dict, event: Optional[MenuEvent]) -> None:
        self.log_path.parent.mkdir(parents=True, exist_ok=True)
        record = TouchBridgeEvent(payload=payload, menu_event=asdict(event) if event else None, timestamp=time.time())
