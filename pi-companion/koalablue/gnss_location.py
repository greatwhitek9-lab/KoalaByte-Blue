...
def locked_location_dict(reason: str = "password_required") -> dict[str, object]:
    return {
        ...
        "note": "location fields are present by default, but coordinates are not written until the operator unlocks the protected-actions password gate",
    }
...
def current_fix(*, authorized: Optional[bool] = None, meshtastic_port: Optional[str] = None, saved_path: str | Path = DEFAULT_FIX_PATH, password: Optional[str] = None, prompt: bool = False) -> Optional[GnssFix]:
    if not location_logging_authorized(enabled=authorized, password=password, prompt=prompt):
        return None
    return env_fix() or saved_fix(saved_path) or (meshtastic_info_fix(meshtastic_port) if meshtastic_port else None)
