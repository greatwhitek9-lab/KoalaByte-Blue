...
def write_ear_tag_tx_lab_plan(output_dir: str | Path = "logs/koalabyte_lab") -> Path:
    root = Path(output_dir)
    root.mkdir(parents=True, exist_ok=True)
    now = time.time()
    stamp = time.strftime("%Y%m%d_%H%M%S", time.localtime(now))
    plan = EarTagTxLabPlan(
        action="KoalaByte Lab",
        mode="synthetic_owned_lab_ble_advertisement",
        created_at=now,
        device_name="KoalaByte Lab",
    ...
    out = root / f"koalabyte_lab_plan_{stamp}.json"
...
def run_cli() -> int:
    ...
    parser = argparse.ArgumentParser(description="Write a KoalaByte Lab setup plan")
    parser.add_argument("--output-dir", default="logs/koalabyte_lab")
    args = parser.parse_args()
    path = write_ear_tag_tx_lab_plan(args.output_dir)
    print(json.dumps({"action": "KoalaByte Lab", "plan_path": str(path)}, indent=2, sort_keys=True))
    return 0
