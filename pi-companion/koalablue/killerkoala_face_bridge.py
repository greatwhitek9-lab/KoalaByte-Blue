...
def emit_face(state: str, message: str = "", *, enabled: bool = True, duration_ms: int = 4500) -> dict:
    ...
    heltec_port = os.getenv("KOALABYTE_HELTEC_FACE_PORT", os.getenv("HELTEC_PORT", "")).strip()
    baud = int(os.getenv("KOALABYTE_FACE_BAUD", str(DEFAULT_BAUD)))
    wrote_esp32 = False if os.getenv("KOALABYTE_FACE_DISABLED") else _serial_write(esp32_port, baud, payload)
    wrote_heltec = False if os.getenv("KOALABYTE_FACE_DISABLED") else _serial_write(heltec_port, baud, payload)
    DEFAULT_LOG_DIR.mkdir(parents=True, exist_ok=True)
    result = {"payload": payload, "esp32_port": esp32_port, "heltec_port": heltec_port, "wrote_esp32": wrote_esp32, "wrote_heltec": wrote_heltec}
