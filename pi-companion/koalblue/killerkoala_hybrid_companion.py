...
def _system_prompt() -> str:
    return (
        ...
        "Safety: only authorized lab diagnostics, defensive/educational Bluetooth workflows, status narration, reports, and companion banter. "
        "Never encourage out-of-scope access, theft, disruption, jamming, credential capture, exploitation, or attacking third-party devices. "
        "Keep replies short, varied, and natural. Do not mention that you are an LLM."
    )
...
def should_try_llm(event: str, flexible: bool, config: KillerKoalaLLMConfig) -> bool:
    ...
    if config.mode in {"force", "force_llm", "llm"}:
        return True
    # Default: phrase engine is the fast reliable path. Tiny LLM is only used for explicit flexible banter.
